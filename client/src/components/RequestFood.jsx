...
export default function RequestFood({ item, currentUser }) {
  ...
  useEffect(() => {
    if (order_by?.user && currentUser?.email) {
      setIsOrdered(order_by.user === currentUser.email);
    } else {
      setIsOrdered(false);
    }
  }, [order_by, currentUser?.email]);

  const handleRequestFood = async (e) => {
    e.preventDefault();
    if (!currentUser?.email) return toast.error("Please login first");

    const location_to = e.target.location.value;
    const commentValue = e.target.comment.value;

    if (order_by?.user === currentUser.email) return toast.error("You have already ordered this food");
    if (status === "Not Available") return toast.error("Food is Not Available");

    const addOrder = {
      order_by: { user: currentUser.email, location: location_to, time: Date.now() },
      comment: commentValue,
      status: "Not Available",
    };

    try {
      await axios.patch(`${import.meta.env.VITE_SERVER_URL}/add-order/${_id}`, addOrder);
    ...
    <>
      ...
      <dialog id="request_food" className="modal modal-bottom sm:modal-middle">
        <div className="modal-box">
          ...
          <div className="flex flex-col items-center justify-between border-[1px] border-secondary p-2 rounded-lg">
            ...
            <div className="flex flex-col items-center gap-2 py-2">
              ...
              <span className="text-base-600">
                For <span className="font-semibold ml-1">{quantity}</span> Peoples
              </span>
              <span className="text-base-600">
                Location: <span className="font-semibold ml-1">{pickup_location}</span>
              </span>
            </div>
          </div>
          ...
          {authorData && (
            <>
              <div className="text-secondary w-full my-4">Donated By :</div>
          ...
          <form onSubmit={handleRequestFood} className="w-full mt-4">
            <input
              className="input w-full input-secondary z-9 focus-within:outline-offset-0 my-2"
              type="text"
              name="location"
            ...
            <fieldset className="fieldset w-full">
              ...
              <textarea
                ...
                placeholder="Information about this food."
